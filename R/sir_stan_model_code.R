...
# ------------------
  ...
  if ( selection=="discrete_autoregressive_with_observation_error" ) {
    return(
...
parameters {
  ...
  real<lower=-0.1, upper =0.1>  MSErrorI;  // fractional mis-specification error due to latent, asymptompatic cases, reporting irregularities
  real<lower=-0.1, upper =0.1>  MSErrorR;  // fractional mis-specification error due to variable disease progression/secondary infections
...
model {
  ...
  //Isd ~ beta(0.5, 0.5);
  //Rsd ~ beta(0.5, 0.5);
  ar1 ~ cauchy( 0.5, 1.0 ); // autoregression
  ...
  //MSErrorR ~ cauchy( 0.5, 1.0 );  // proportion of R that are misclassified

  BETA[1] ~ normal( BETA_prior, BETA_prior/3.0 );  // # 10% CV
  for (i in 1:(Nobs)) {
...
# ------------------
  ...
  if ( selection=="discrete_autoregressive" ) {
    return(
...
model {
  ...
  ar1sd ~ beta(0.5, 0.5);
  ar1k ~ cauchy( 0.5, 1.0 );
  GAMMA ~ normal( GAMMA_prior, GAMMA_prior/4.0 );  // recovery of I ... always < 1
  sir0 ~ beta(0.5, 0.5) ;


  BETA[1] ~ normal( BETA_prior, BETA_prior/4.0 );  // # 10% CV
  for (i in 1:(Nobs)) {
    BETA[i+1] ~ normal( ar1k + ar1 * BETA[i], ar1sd );
  }
  for ( i in (Nobs+1):(Ntimeall-1) ) {
